...
def test_classify_lines(fixture_files, tmp_path):
    ...
    exit_code = fizzbuzz.main([fixture_files['classify_in.txt'].as_posix(), out_file.as_posix(), "classify"])
...
def test_filter_fizz_lines(fixture_files, tmp_path):
    ...
    exit_code = fizzbuzz.main([fixture_files['classify_expected.txt'].as_posix(), out_file.as_posix(), "filter", "--substring", "fizz"])
    assert not exit_code
...
def test_filter_buzz_lines(fixture_files, tmp_path):
    ...
    exit_code = fizzbuzz.main([fixture_files['filter_fizz_expected.txt'].as_posix(), out_file.as_posix(), "filter", "--substring", "buzz"])
    assert not exit_code
